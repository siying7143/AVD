...
class JVNSourceImporter(BaseSourceImporter):
    ...
    def _published_years_for_target_cve_year(self, target_year: int) -> List[int]:
        # Include neighbouring publication years because JVN may publish advisories
        # before or after the CVE year embedded in the identifier.
        """
        Stable strategy: always fetch spillover records from the next calendar year.
        This prevents historical reruns from losing JVN advisories in target_year + 1
        that still belong to the target CVE year.
        """
        current_year = datetime.now().year
        years = {target_year}
        if target_year + 1 <= current_year:
            years.add(target_year + 1)
        return sorted(years)

    def import_year(self, year: int) -> Iterable[dict]:
        # Combine feed, overview, and detail data, then emit only CVEs from the
        # requested target year.
        advisories: Dict[str, AdvisoryAggregate] = {}
        published_years = self._published_years_for_target_cve_year(year)

        self._log(
            f"[INFO] JVN target_cve_year={year}: import started "
            f"(published_years={published_years})"
        )

        # 1) Feed stage: include spillover years as well.
        for published_year in published_years:
            for lang in self.LANGS:
                for template in self.FEED_URL_TEMPLATES:
                    url = template.format(lang=lang, year=published_year)
                    try:
                        xml_text = self.get_bytes(url).decode("utf-8", errors="replace")
                    except Exception as exc:
                        print(
                            f"[WARN] JVN target_cve_year={year}: feed fetch failed: {url} ({exc})",
                            flush=True,
                        )
                        continue
                    ...
                    self._log(
                        f"[INFO] JVN target_cve_year={year}: "
                        f"feed published_year={published_year} lang={lang} "
                        f"added={added} advisories_total={len(advisories)}"
                    )

        # 2) Overview API stage: include spillover years as well.
        for published_year in published_years:
            for lang in self.LANGS:
                self._log(
                    f"[INFO] JVN target_cve_year={year}: overview "
                    f"published_year={published_year} lang={lang} start "
                    f"advisories_total={len(advisories)}"
                )
                self._fetch_overview_pages(
                    advisories,
                    target_cve_year=year,
                    published_year=published_year,
                    lang=lang,
                )
                self._log(
                    f"[INFO] JVN target_cve_year={year}: overview "
                    f"published_year={published_year} lang={lang} done "
                    f"advisories_total={len(advisories)}"
                )
        ...
        self._log(
            f"[INFO] JVN target_cve_year={year}: detail enrichment start "
            f"advisories_total={total_advisories}"
        )
        detail_hits = 0
        ...
        for idx, (advisory_id, agg) in enumerate(advisories.items(), start=1):
            ...
            if idx == 1 or idx % 100 == 0 or idx == total_advisories:
                ...
                self._log(
                    f"[INFO] JVN target_cve_year={year}: detail progress "
                    f"{idx}/{total_advisories} detail_hits={detail_hits} "
                    f"scored_advisories={scored} product_advisories={with_products}"
                )

        # 4) Emit records: only output CVEs from the requested target year.
        emitted_records = 0
        scored_records = 0
        product_records = 0
    ...
    def _fetch_overview_pages(
        ...
        # Page through the JVN overview API until all available matching records are collected.
        start_item = 1
        page_size = 50
        max_pages = 400
        ...
        for page_no in range(1, max_pages + 1):
            params = {
                ...
                "datePublishedStartY": published_year,
                "datePublishedStartM": 1,
                "datePublishedStartD": 1,
                "datePublishedEndY": published_year,
            ...
            try:
                ...
                print(
                    f"[WARN] JVN target_cve_year={target_cve_year}: "
                    f"overview fetch failed: {url} ({exc})",
                    flush=True,
                )
                break

            status_first, status_returned, status_total, raw_item_count = self._extract_overview_status(xml_text)
            ...
            if page_no == 1 or page_no % 5 == 0 or raw_item_count == 0 or added == 0:
                self._log(
                    f"[INFO] JVN target_cve_year={target_cve_year}: "
                    f"overview published_year={published_year} lang={lang} "
                    f"page={page_no} start_item={start_item} "
                    f"raw_items={raw_item_count} added={added} advisories_total={len(advisories)}"
                )

            # Do not stop early just because this page has no matches for target_cve_year.
            # Spillover-year records, such as CVE-2025-* on 2026 pages, often appear on later pages.
            if raw_item_count == 0:
                break

            if (
                status_first is not None
                and status_returned is not None
                and status_total is not None
                and (status_first + status_returned - 1) >= status_total
            ):
                break
    ...
    def _fetch_detail(self, advisory_id: str, lang: str) -> Optional[dict]:
        # Fetch per-advisory detail XML to enrich overview records with CVSS, dates, and references.
    ...
    def _extract_overview_status(self, xml_text: str) -> Tuple[Optional[int], Optional[int], Optional[int], int]:
        try:
            root = ET.fromstring(xml_text)
        except Exception:
        ...
        for elem in root.iter():
            local_name = self._local_name(elem.tag).lower()
            if local_name in {"item", "entry", "vulinfo"}:
                raw_item_count += 1
            if local_name == "status":
                try:
                    status_first = int(elem.attrib.get("firstRes")) if elem.attrib.get("firstRes") else None
                except Exception:
                    status_first = None
                try:
                    status_returned = int(elem.attrib.get("totalResRet")) if elem.attrib.get("totalResRet") else None
                except Exception:
                    status_returned = None
                try:
                    status_total = int(elem.attrib.get("totalRes")) if elem.attrib.get("totalRes") else None
                except Exception:
                    status_total = None

        return status_first, status_returned, status_total, raw_item_count
    ...
    def _merge_detail_payload(self, agg: AdvisoryAggregate, detail: dict, *, lang: str) -> None:
        # Merge multilingual detail responses into one advisory aggregate without losing stronger values.
